...
- 요청을 한 번에 하나 처리한다. CPU 배포는 모델 처리 제한을 300초, 백엔드 읽기 제한을 310초로 설정한다. 기본 개발 환경의 모델 제한은 90초다. 작은 합성 세트의 성공을 운영 성능으로 일반화하지 않는다.
...
- 공개 체험: https://wecall.34.60.91.223.sslip.io/#/demo
- us-central1-c의 e2-standard-4에서 실제 Qwen3 4B 추출 성공. 합성 데이터로 추출 승인, 대상 판정, 입고 증거 승인·재판정, 대응 작업 완료, 보고서 검증까지 102초 소요. 단일 실행 전체 시간이며 모델 단독 지연이나 동시 사용자 성능을 뜻하지 않는다.
- 첫 실행은 기존 90초 제한으로 실패했고, CPU용 제한 조정 후 재검증에 성공했다. AI 회귀 테스트 35개 통과.
- Vercel 연결은 GitHub 재인증 대기 중이다. 현재 공개 주소는 GCP에서 프론트도 제공한다.
