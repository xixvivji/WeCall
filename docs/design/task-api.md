...
담당자는 활성 계정 username, actor는 로그인 계정에서 결정한다. [역할별 접근 제어](auth-api.md)가 적용되며 새 증빙 생성 시 파일 첨부는 [첨부 API](attachments-api.md)로 지원하며 자동 작업 생성은 후속 범위다. 사건 종료는 [종료 API](closure-api.md)로 구현했다.
...
- COMPLETED/CANCELLED에서는 배정·증빙 추가 불가. CANCELLED는 재개 API를 제공하지 않지만 미검토 증빙을 승인/거절해 정리할 수 있다. 종료된 사건의 작업은 재개 전까지 변경 불가.
