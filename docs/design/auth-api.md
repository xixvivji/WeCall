...
username 변경·계정 삭제·역할 변경·타인 비밀번호 초기화 API는 아직 없다.
...
로컬 기본 바인딩은 127.0.0.1이다. HTTPS 배포 시 SESSION_COOKIE_SECURE=true를 사용한다. 배포용 TLS, 로그인 시도 제한, 계정 복구, 세션 공유, 프론트 CORS/배포 도메인 설정은 별도 작업이다.
...
검토자는 프론트의 계정 관리 메뉴에서 계정 목록과 신규 계정 발급을 사용할 수 있다. 실행 담당자는 메뉴와 생성 폼에 접근할 수 없으며 서버에서도 기존 역할 검사를 유지한다. 비활성화·재활성화와 변경 이력 조회도 제공한다. 두 역할 모두 내 계정 메뉴에서 본인 비밀번호를 변경할 수 있다.

## 비밀번호 변경과 계정 상태

- `POST /api/auth/password`: 두 역할 모두 본인만 변경. `currentPassword`, `newPassword`를 보내며 성공하면 204와 현재 세션 로그아웃. 새 비밀번호는 12자 이상·UTF-8 72바이트 이하이며 기존 값과 달라야 한다.
- `POST /api/users/{username}/status`: REVIEWER 전용. `enabled`, 목록의 `version`을 복사한 `expectedVersion`, 필수 `note`를 전달한다. 오래된 버전·동일 상태 요청은 409. 본인과 마지막 활성 검토자의 비활성화를 막으며 검토자 상태 변경을 트랜잭션 잠금으로 직렬화한다.
- `GET /api/users/{username}/events`: REVIEWER 전용. 비밀번호 변경·비활성화·재활성화 이력 최근 100건. 행위자와 사유를 기록하며 비밀번호·해시는 반환하거나 기록하지 않는다.

로그인 시 보안 버전을 세션에 보관하고 이후 요청마다 DB의 활성 여부와 버전을 확인한다. 비밀번호 변경·비활성화·재활성화 모두 버전을 증가시키므로 다른 세션은 다음 요청에서 401 `SESSION_REVOKED`로 무효화된다. 재활성화해도 이전 세션은 복구되지 않는다. 이미 진행 중인 요청을 소급 취소하지 않으며 세션 공유 저장소는 아직 없다.

비활성화는 과거 업무 기록을 삭제하거나 미완료 작업을 자동 재배정하지 않는다. 검토자가 별도 재배정해야 한다. 변경과 감사 이력 저장은 같은 트랜잭션이다.
