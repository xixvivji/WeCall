...
`GET /api/v1/datasets/{id}/provenance`는 두 역할 모두 로그인 후 조회할 수 있다. 없는 데이터는 404다. source는 CSV/EVIDENCE/LEGACY, files는 해당 버전에 직접 업로드한 파일 메타데이터, derivation은 증거 승인으로 만든 버전의 기준 데이터·사건·증거·승인자·승인 시각이다.

새 HTTP 등록은 로그인 계정을 uploadedBy로 기록하며 요청 본문에서 등록자를 받지 않는다. 내부 테스트·서비스의 등록자 미지정 호출은 null을 보존한다. 파일별 유형, 표시 파일명, SHA-256, 실제 읽은 바이트 수, 헤더 제외 검증 데이터 행수를 저장한다. 해시는 BOM·개행을 포함한 업로드 바이트 기준이며 파싱된 CSV를 재직렬화해 계산하지 않는다. 표시 파일명은 디렉터리 부분을 제거하고 제어문자를 치환하며 파일명이 없으면 null이다. 이 메타데이터는 데이터와 동일 트랜잭션으로 저장한다.
