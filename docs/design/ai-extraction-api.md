...
2026-09-16 로컬 Qwen3 4B Instruct 모델을 연결했다. [로컬 AI 실행·검증](local-ai.md)을 참고한다. 기본 provider는 `disabled`이며 모델 미설정으로 실패한다. `fixture`는 지정된 합성 원문에 저장된 정답을 반환하는 모의 구현이다. 임의 문서를 분석하지 않는다. `ollama-local`이 실제 로컬 모델 Provider 계약을 구현하며 v1 응답을 유지한다.
...
DB 작업 선점 트랜잭션 후 외부 호출을 수행한다. 연결 제한 2초, 기본 읽기 제한 10초(로컬 실행은 환경 설정으로 100초), 응답 상한 256KiB다. 이 제한은 읽기 전체의 절대 실행시간 제한은 아니다. RUNNING이 2분을 넘으면 다음 워커 점검 시 WORKER_INTERRUPTED로 남기고 늦은 결과로 덮어쓰지 않는다. 자동 재시도는 하지 않는다. 현재 워커는 서버당 순차 처리한다.
