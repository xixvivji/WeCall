...
[세션 로그인·역할별 권한](auth-api.md)이 적용된다. 승인자는 로그인한 검토자 계정으로 기록한다.
...
이 스크립트는 매번 **새 합성 데이터 버전·사건·조건·승인·판정**을 생성한다. 사람이 사전에 작성한 조건·상품 정답을 demo-reviewer 계정으로 승인하는 시연이다. 실제 고객 승인 절차를 자동화한 것이 아니다.
...
승인 요청: `{}`. 로그인 검토자를 사용하므로 reviewer 입력은 생략한다.
