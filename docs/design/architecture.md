...
기준: 2026-09-13, 파일 첨부 V10 반영. 로컬 개발 구성이다. 운영 배포도나 회사가 승인한 보안 경계가 아니다.
...
flowchart LR
    ...
    B --> A[AttachmentStorage 인터페이스]
    A --> L[전용 로컬 디렉터리 / UUID 파일]
...
파일 첨부는 Spring의 AttachmentStorage 인터페이스 뒤에 로컬 어댑터를 둔다. 공개 정적 경로를 만들지 않고 로그인·업무 권한을 검사한 다운로드만 제공한다. 실제 저장소·보관 정책은 [파일 관리 초안](file-data-policy.md)에 구분했다. 로컬 저장소는 구현했으며 운영 객체 저장소·암호화·백업은 별도다. [첨부 API와 복구 절차](attachments-api.md)
