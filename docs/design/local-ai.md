...
- 보수적 사전 차단: 제외·미만·초과·이상·이하·아닌·빼고, 일부 연도 미확인·회수 취소/철회·명령 표현, 여러 번 값이 제시된 제조번호/소비기한 항목은 수동 검토로 넘긴다. 날짜 직후의 이전/이후는 열린 범위로 차단하지만 “접수 이후 연락”은 차단하지 않는다. 같은 상품을 반복 설명하는 정상 문서도 거절될 수 있으며 모든 부정·다중 상품 표현을 탐지하는 것은 아니다.
- 모델은 원문 근거의 시작/끝 줄 번호만 선택한다. 서버가 원본 문자열을 그대로 연결해 인용문을 만든다. 모델이 근거를 다시 쓰지 않는다.
- 두 필드 사이에 명확한 이거나/또는 또는 이면서/이며/이고가 있으면 마지막 연결어와 AND/OR 결과가 일치하는지 확인한다. 불일치하면 자동 교정하지 않고 수동 검토로 넘긴다.
...
기존 개발9건 외에 확대18건과 별도8건을 추가했다. [세트와 평가 기준](../../ai/evaluation/README.md)을 따른다. 일부 거절은 모델 추론 전에 규칙으로 차단하므로 모델 단독 정확도와 구분한다. 개발 중 이 세트로 프롬프트를 조정했으며 독립적인 실무 평가 세트가 아니다. 모델 평가 결과는 Git 제외 `validation-results/local-ai-<suite>-<label>.json`에 기록된다. 과거 고정 이름의 실행 결과는 기존 기록으로 남아 있다. 기대 조건과 추출 조건의 값·경계·누락값 조합에서 판정 일치를 검사한다.
...
평가 확대와 local-extract-v2 변경 결과는 [2026-09-16 확대 평가](../validation/ai-evaluation-expansion-2026-09-16.md)에 기록했다.
