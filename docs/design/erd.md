...
상태: 설계 초안. DB 제품·물리 스키마·마이그레이션은 아직 확정/구현하지 않았다.
...
DB 선정 후 물리 ERD·Flyway 스키마를 확정한다. 첫 API는 CSV 검증·등록, 사건·수동 조건 등록, 조건 승인, 판정 실행, 재고·출고 영향 조회다. 인증과 승인 권한은 실제 사용자 데이터 적용 전에 구현한다.
