...
기준일: 2026-09-17 · 코드 기준: develop, Flyway V12. ‘구현’은 개발·합성 검증 기준이며 현업 인수나 운영 배포 완료를 뜻하지 않는다.
...
| DOC-03 | 원문 변경과 AI | 이전 원문 버전의 분석을 새 조건으로 전환하지 않음; 과거 결과 유지 | 구현 / ExtractionIntegrationTests; 기존 승인 조건 재검토 관리는 미구현 |
...
PDF 원본·원문 버전을 보존하고 오래된 AI 결과 전환은 차단한다. 이미 생성된 조건·승인·판정의 재검토 상태 관리는 아직 없다. AI는 제조번호·소비기한의 제한된 조건을 다루며 원문 6,000바이트·80줄 초과, 복잡한 예외·상품별 조건은 수동 검토가 필요하다. ClamAV는 어댑터만 검증했으며 실제 엔진 운용은 별도다.

**미구현 보완안과 완료 기준은 [기능 보완 설계](design/product-roadmap.md)에만 BACKLOG로 표시한다.** 원문 변경 후 재검토, 분석 오류 설명, 대표 문서 검증을 우선 제안한다. 작업 기한·우선순위와 상품 후보 비교는 그 다음이다. 문서 보완을 기능 착수 승인이나 완료로 취급하지 않는다.
