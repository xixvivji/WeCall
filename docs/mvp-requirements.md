...
**미구현 보완안과 완료 기준은 [기능 보완 설계](design/product-roadmap.md)에만 BACKLOG로 표시한다.** 원문 변경 후 재검토는 구현했으며, 분석 오류 설명과 대표 문서 검증을 다음으로 제안한다. 작업 기한·우선순위와 상품 후보 비교는 그 다음이다. 문서 보완을 기능 착수 승인이나 완료로 취급하지 않는다.
