...
기준일: 2026-09-14. 코드 기준: 첨부 검사 V11 구현 반영. 이 문서의 ‘구현’은 합성 데이터로 기능과 테스트를 확인했다는 뜻이며, 실사용 고객 검증이나 운영 배포 완료를 뜻하지 않는다.
...
테스트 소스: [백엔드](../backend/src/test/java/com/wecall/), [프론트](../frontend/e2e/). 최신 보고서가 없는 환경에서는 상태를 재검증한다. 현재 기준 마지막 로컬 검증은 백엔드 114건 통과, 프론트 로컬 24건 통과 후 추가 영향 검증 3건 통과, 프론트 빌드 성공, 고아 파일 후보 검사 1건 통과다. 문서 작성만으로 테스트를 새로 실행했다고 보지 않는다.

사건별 대응 결과 보고서와 인쇄·PDF 저장을 구현했다. [보고서 API·검증](design/case-report-api.md)을 참고한다.
...
4. 프론트·AI 후속 개발은 [백엔드 인수 문서](validation/backend-handoff.md)를 따른다.
5. 배포 재개 시 운영 저장소·실제 검사 엔진 배치·DB와 파일 백업 복구·보관 정책을 준비하고 release 브랜치에서 검증한다.
...
사건 탭·기준 판정 URL 유지와 링크 복사를 구현했다. [화면 이동 계약](design/case-navigation.md)을 따른다.

입력 보호와 사건별 다음 할 일 안내를 구현했다. [입력 보호·업무 안내 계약](design/workflow-guidance.md)을 따른다.
...
최신 로컬 검증(2026-09-16): 백엔드114건, Python AI 계약25건, 프론트26건 통과. 실제 로컬 모델을 포함한 합성 업무 연동과 작은 개발 평가9건도 통과했다. [평가 범위와 한계](validation/local-ai-2026-09-16.md)를 참고한다.

로컬 AI 평가를 합성35건으로 확대하고 OR 혼동·상품별 조건 병합·상충/명령 문서 처리·과도한 거절을 보완했다. Python 계약·평가 테스트35건 통과. [수정 전후 결과와 한계](validation/ai-evaluation-expansion-2026-09-16.md)를 따른다.
