...
PostgreSQL 기반 CSV 등록, 회수 사건·조건 등록·승인, 대상 판정과 영향 조회를 구현했습니다. 입고 증거 텍스트 등록·승인·누락값 보완·재판정까지 구현했습니다. 담당자 배정·대응 작업·텍스트 처리 증빙·완료/재개 이력도 구현했습니다. 사건 종료 점검·승인·재개와 당시 점검 결과 보존도 구현했습니다. 세션 로그인과 검토자·실행 담당자 권한도 적용했습니다. FastAPI 조건 추출 요청·결과 저장·검토용 조건 초안 전환을 구현했습니다. 로컬 Ollama 모델 기반 조건 추출과 사람 검토 화면을 구현했습니다. OCR은 아직 연결하지 않았습니다.
...
- 파일 저장소: 로컬 전용 디렉터리 구현; 운영 저장소·AI 모델·배포 환경은 추후 결정

Spring Boot는 업무 로직·조건 판정·승인·데이터 저장을 맡고, FastAPI는 문서 해석·조건 추출·상품 후보 비교를 담당할 예정입니다. 두 서버 간 조건 추출 호출과 로컬 모델을 연결했습니다. 기본값은 disabled이며 로컬 실행 도구에서 활성화합니다.
...
- [텍스트 PDF 원문 가져오기](docs/design/source-pdf.md)
