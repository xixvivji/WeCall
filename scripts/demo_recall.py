...
def verify_rows(result, phase):
    # Use the independently authored row answers, not totals generated by the API.
    with (SAMPLE / "expected_receipt_decisions.csv").open() as f:
        decisions = {r["receipt_id"]: r[phase] for r in csv.DictReader(f)}
    actual = {item["receiptId"]: item["decision"] for item in result["inventory"]}
    if actual != decisions:
        raise SystemExit(f"Receipt row mismatch: {phase}")
    with (SAMPLE / "expected_shipment_impacts.csv").open() as f:
        expected = {r["shipment_id"]: {key: int(r[f"{phase}_{column}"])
                    for key, column in (("target", "target"), ("nonTarget", "non_target"), ("needsReview", "needs_review"))}
                    for r in csv.DictReader(f)}
    actual = {r["shipmentId"]: {k: r[k] for k in ("target", "nonTarget", "needsReview")} for r in result["shipments"]}
    if actual != expected:
        raise SystemExit(f"Shipment row mismatch: {phase}")
...
def main():
    ...
    report_assessment = after if args.with_evidence else result
    report = request(prefix + f"/report?assessmentId={report_assessment['id']}")
    if report["assessment"] != report_assessment:
        raise SystemExit("Report changed the selected immutable assessment")
    if "UNRESOLVED_SHIPMENTS" not in {b["code"] for b in report["blockers"]}:
        raise SystemExit("Report lost the unresolved shipment blocker")
    output["rowAnswersVerified"] = True
    output["reportVerified"] = True
    client.logout()
